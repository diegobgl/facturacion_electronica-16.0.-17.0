...
class POS(models.Model):
    ...
    def do_dte_send_order(self):
        ids = []
        for order in self:
            if not order.document_class_id or order.document_class_id.es_factura():
                continue
    ...
    def _totales(self, resumen):
        ...
        Totales = dict(MntTotal=0, MntNeto=0, MntExe=0, IVA=0, TasaIVA=0,
            MontoNF=0)
        ...
        if MntExe < 0:
            MntExe *= -1
        if self.amount_tax == 0 and self.document_class_id.es_boleta_afecta():
            raise UserError("Debe ir almenos un Producto Afecto")
        Neto = 0
        for l in resumen['Detalle']:
            if l.get('IndExe'):
                continue
            Neto += l['MontoItem']
        if Neto:
            Neto = Neto / (1 + (resumen['TasaIVA'] /100.0))
        Totales['MntNeto'] = currency.round(Neto)
        Totales['TasaIVA'] = resumen['TasaIVA']
        Totales['IVA'] = self.amount_tax
        if MntExe > 0:
        ...
        VlrPagar = Totales['MntTotal'] + MontoNF
    ...
    def _get_lines(self):
        return self.with_context(lang="es_CL").lines

    def _get_rounding(self, amount_total, nro_lin_dte):
        rounding = self.amount_paid - amount_total
        if rounding:
            return [ rounding,
                    {
                        'NroLinDet': nro_lin_dte,
                        'NmbItem': self.config_id.rounding_method.name,
                        'QtyItem': 1,
                        'PrcItem': rounding if rounding > 0 else rounding * -1,
                        'MontoItem': rounding if rounding > 0 else rounding * -1,
                        'IndExe': 2 if rounding > 0 else 6
                    }]
        return False
    ...
    def _invoice_lines(self):
        ...
        TasaIVA = 0
        amount_total = 0
        order_lines = self._get_lines()
        for line in order_lines:
            IndExe = 0
            no_product = line.product_id.default_code != 'NO_PRODUCT'
            lines = {}
            ...
            if line.product_id == self.session_id.config_id.tip_product_id:
                ...
                for t in line.tax_ids_after_fiscal_position[0]:
                    if t.sii_code not in [0, 14, 15]:
                        continue
                    if t.ind_exe:
                        IndExe = t.ind_exe
...
#                lines['RUTMandante']
            ...
            if product:
                ...
                if qty == 0:
                    lines['QtyItem'] = 1
                uom_name = line.product_id.uom_id.with_context(
                ...
                if not taxInclude:
                    MontoItem = currency.round(line.price_subtotal)
                else:
                    MontoItem = currency.round(line.price_subtotal_incl)
            amount_total += MontoItem
            if MontoItem < 0:
                MontoItem *= -1
            if IndExe:
                lines['IndExe'] = IndExe
        ...
        rounding = self._get_rounding(amount_total, len(order_lines)+1)
        if rounding:
            MontoNF += rounding[0]
            invoice_lines.append(rounding[1])
        return {
